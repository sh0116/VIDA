    ...
    def mousePressEvent(self, event):
        ...
        # Goma
            ...
            if event.button() == Qt.LeftButton or event.button() == Qt.RightButton:
                ...
                self.image().paintPoint(x, y, color, self.context.eraserSize)
            ...
            if event.button() == Qt.LeftButton or event.button() == Qt.RightButton:
                ...
                color = QtGui.QColor(255, 255, 255)
                self.image().paintPoint(x, y, color, self.context.eraserSize)
                self.context.custom_X,self.context.custom_Y = x,y
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.context.currentTool == Pixeler.Tools.Selection and event.button() == QtCore.Qt.LeftButton:
            ...
            if self.selecting:
                ...
                print("start point x,y and end point x,y : "+"(" + str(self.image().selection.origin.x()) + ", " + str(self.image().selection.origin.y()) + ")"+"(" + str(int(x)) + ", " + str(int(y)) + ")")

                self.context.custom_selection_X1,self.context.custom_selection_Y1 = min([self.image().selection.origin.x(),int(x)]),min([self.image().selection.origin.y(),int(y)])
                self.context.custom_selection_X2,self.context.custom_selection_Y2 = max([self.image().selection.origin.x(),int(x)]),max([self.image().selection.origin.y(),int(y)])
