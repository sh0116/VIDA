...
class Preview (QtWidgets.QDockWidget):
	...
	def update(self):
		...
		if self.context.currentImage() != None:
			if self.context.currentQImage().width() > 1280 or self.context.currentQImage().height() > 1280:
				imatge = self.context.currentQImage().scaled(1280, 1280, Qt.KeepAspectRatio)
