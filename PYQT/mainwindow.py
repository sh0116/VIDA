...
class MainWindow(QtWidgets.QMainWindow):
	...
	def __init__(self, context, signals):
		...
		self.resize(800,480)
		self.setWindowTitle(self.context.getText("KBSC", "title"))
		self.setWindowIcon(QtGui.QIcon('blueprint.png'))
	...
	def createToolBarActions(self):
		...
		'''
		connects = [lambda: self.context.changeCurrentTool(Pixeler.Tools.Pencil),
	...
	def createFileActions(self):
		...
		'''
		ids = ["open", "saveas"]
		icons = ["document-open.png", "document-save-as.png"]
		shortcuts = ['Ctrl+O','Ctrl+Shift+S']
		connects = [self.openFile, self.saveFileAs]
	...
	def createDockWidgets(self):
		...
		#input tag widget
		self.tagging = Tagging(self.context.getText("dock_widgets", "tool_properties"), self.context, self.signals)
		self.addDockWidget(Qt.RightDockWidgetArea, self.tagging)
		self.tagging.setSizePolicy(QtWidgets.QSizePolicy.Preferred, QtWidgets.QSizePolicy.Expanding)
	...
	def saveFileAs(self):
		...
		fileName, filterName = d.getSaveFileName(self,
					...
					"", 
					"*.bmp;;*.gif;;*.png;;*.xpm;;*.jpg","*.obj")

		if fileName.split(".")[-1] in ["bmp", "gif", "png", "xpm", "jpg","obj"]:
